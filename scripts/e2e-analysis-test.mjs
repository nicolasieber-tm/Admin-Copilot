// E2E-Test der Analyse-Pipeline: meldet sich als Testnutzer an (anon-API,
// RLS greift), lädt eine synthetische Rechnung (PDF) hoch und wartet auf
...
// Minimales, aber wohlgeformtes PDF mit dem Text einer Beispielrechnung
function buildTestPdf() {
  const textLines = [
    ...
    "",
    "Bitte bezahlen Sie den Betrag bis zum Faelligkeitsdatum.",
...
const { data: doc, error: docError } = await supabase
  ...
  .insert({
    ...
    title: "E2E-Test Prämienrechnung",
...
if (pageError) throw new Error(`Seite: ${pageError.message}`);

// Letzter Schritt löst den Enqueue-Trigger aus
const { error: finishError } = await supabase
  .from("documents")
  .update({ storage_path: `${workspaceId}/${doc.id}` })
  .eq("id", doc.id);
if (finishError) throw new Error(`Abschluss: ${finishError.message}`);
...
while (Date.now() < deadline) {
  ...
  const { data: current } = await supabase
    .from("documents")
    .select("status, category, sender_name, document_date, analysis_confidence")
    .eq("id", doc.id)
    .single();
  console.log("  Status:", current?.status);
  if (current?.status === "ready_for_review" || current?.status === "failed") {
    finalStatus = current;
    break;
  }
}
...
if (!finalStatus) {
  console.log("TIMEOUT: keine Analyse innert 150 s");
  process.exit(2);
}
...
const { data: analysis } = await supabase
  ...
  .select("status, provider, model, prompt_version, error_code, error_message, explanation_result")
...
const { data: entities } = await supabase
  ...
  .select("entity_type, value_text, source_text, page_number, confidence")
  .eq("document_id", doc.id);
console.log("\n=== Entities ===");
console.log(JSON.stringify(entities, null, 2));

process.exit(finalStatus.status === "ready_for_review" ? 0 : 1);
